...
public class StatcvsXMLEngine implements StatcvsEngine
{
    ...
    public void generateStats()
    {
        try
        {
            // Once a ClassWorld is created, realms within it can be created. These
            // realms effectively only allow loading of the core JVM classes.

            ClassWorld world = new ClassWorld();

            ClassRealm statcvsRealm = world.newRealm("statcvs");

            // In order to make each ClassRealm useful, constituent must be added to
            // that each can provide certain classes.

            //File f = new File("lib/statcvs-xml.jar");
            //System.out.println(FileUtil.toString(f));
            
            statcvsRealm.addConstituent(new File("lib/commons-logging.jar").toURL());
            //statcvsRealm.addConstituent(new File("lib/log4j.jar").toURL());
            statcvsRealm.addConstituent(new File("lib/statcvs-xml/statcvs-xml-0.9.2.jar").toURL());
            statcvsRealm.addConstituent(new File("lib/statcvs-xml/jdom-b9.jar").toURL());
            statcvsRealm.addConstituent(new File("lib/statcvs-xml/jfreechart-0.9.16.jar").toURL());
            statcvsRealm.addConstituent(new File("lib/statcvs-xml/jcommon-0.9.1.jar").toURL());
            statcvsRealm.addConstituent(new File("lib/statcvs-xml/commons-jexl-1.0-beta-1.jar").toURL());
            ...
            //statcvsRealm.importFrom("jdom", "*");

            // The container implementation can then be loaded from it's realm and
            // used.

            //statcvsRealm.findResources()
            
            Thread.currentThread().setContextClassLoader(statcvsRealm.getClassLoader());
            
            System.out.println("Log classloader = " + 
                statcvsRealm.getClassLoader().loadClass(
                    "org.apache.commons.logging.Log").getClassLoader());
                
            System.out.println("NoOpLog classloader = " + 
                statcvsRealm.getClassLoader().loadClass(
                    "org.apache.commons.logging.impl.Log4JLogger").getClassLoader());
                
            System.setProperty(
                "org.apache.commons.logging.Log",
                "org.apache.commons.logging.impl.SimpleLog"); //"org.apache.commons.logging.impl.Log4JLogger");
            ...
            // MyContainer container = (MyContainer) statcvsClass.newInstance();
            ...
            //Method m = getAccessibleMethod(statcvsClass, "main", new String[0].getClass());
            //m.invoke(null, new Object[] {args});

            // ReportSettings settings = readSettings(args);
            Method readSettings = getAccessibleMethod(statcvsClass, "readSettings", new String[0].getClass());
            Object settings = readSettings.invoke(null, new Object[] {args});

    		// generateSuite(settings);
            Method generateSuite = getAccessibleMethod(statcvsClass, "generateSuite", settings.getClass());
