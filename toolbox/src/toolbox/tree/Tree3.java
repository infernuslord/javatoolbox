...
public class Tree3 {
    ...
    public static void main(String args[]) throws Exception {
        ...
        try {
            if (rootDir != null) {
                Tree3 t = new Tree3(new File(rootDir), config);
                t.showTree();
            }
        }
     ...
     */
    public Tree3(File rootDir, TreeConfig treeConfig) {
        ...
        if (config.isShowFiles())
            this.fileFilter = new NotFileFilter(DirectoryFileFilter.INSTANCE);
        ...
        if (useRegex) {
            ...
            this.fileFilter = 
                new AndFileFilter(
                    this.fileFilter, 
                    new RegexFileFilter(config.getRegexFilter(), false));  
        }
    }
        ...
        if (config.isShowFiles()) {
            ...
            for (int i = 0; i < files.length; i++) {
                ...
                if (config.isShowFilesize()) {
                    ...
                    String formatted = 
                        FILESIZE_FORMATTER.format(files[i].length());
                ...
                if (config.isShowFileDate()) {
                    out.print("  "); 
                    out.print(DateTimeUtil.formatToSecond(
