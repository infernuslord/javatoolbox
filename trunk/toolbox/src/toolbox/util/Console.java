 ...
 * implementation doesn't understand the command (delegate to a higher authority)
 */
public abstract class Console
{
	 ...
	 * @return String containing the commands results or 
	 *         null/emptystring if not specified
	 */
	...
	{
		/* detach from the input stream for 10 secs so Ctrl-C works and dumps the VM without exiting */
		int numSecs = 10000;
		getPrintStream().println("Detaching from inputstream for " + numSecs/1000 + " secs...");
	...
	protected void commandHelp()
	{
		...
		ps.println("detach     => detaches from System.in so Ctrl-C can trigger a VM dump");		
    ...
    {
    	getPrintStream().println("Free memory  " + Runtime.getRuntime().freeMemory());
    	getPrintStream().println("Total memory " + Runtime.getRuntime().totalMemory());
	}
